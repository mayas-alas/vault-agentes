...
const port=Number(process.env.PORT||4175),host=process.env.HOST||'127.0.0.1',baseUrl=(process.env.AI_BASE_URL||'http://127.0.0.1:31415/v1').replace(/\/+$/,''),model=process.env.AI_MODEL||'auto',transcriptionModel=process.env.AI_TRANSCRIPTION_MODEL||'auto',speechModel=process.env.AI_SPEECH_MODEL||'auto',voice=process.env.AI_VOICE||'alloy',apiKey=process.env.AI_API_KEY||process.env.VAULT_API_KEY,allowPublicAI=process.env.ALLOW_PUBLIC_AI==='true';
...
async function chat(request,response){
  ...
  if(!apiKey)return json(response,503,{error:'AI_API_KEY no esta configurada en el servidor.'});
  try{
    ...
    const upstream=await fetch(`${baseUrl}/responses`,{method:'POST',headers:{authorization:`Bearer ${apiKey}`,'content-type':'application/json'},body:JSON.stringify({model,store:false,max_output_tokens:500,instructions:'Eres Lumen, el agente privado dentro de GNX Vault. Responde en espanol, directo y calido. Ayuda a convertir intenciones en decisiones, acuerdos y siguientes pasos. No inventes conexiones ni acciones ejecutadas. Usa como maximo 90 palabras.',input:[...history,{role:'user',content:message}]})});
    const data=await upstream.json();if(!upstream.ok)throw new Error(data.error?.message||`Gateway ${upstream.status}`);const text=outputText(data);if(!text)throw new Error('La respuesta no incluyo texto.');json(response,200,{text,model:data.model||model,responseId:data.id});
...
async function transcribe(request,response){
  ...
  if(!apiKey)return json(response,503,{error:'AI_API_KEY no esta configurada en el servidor.'});
  try{
    const payload=await body(request,7_500_000),encoded=String(payload.audio||'').replace(/\s/g,''),mime=String(payload.mime||'audio/webm');
    if(!encoded||encoded.length>7_000_000||!/^[A-Za-z0-9+/]+={0,2}$/.test(encoded))return json(response,400,{error:'Audio invalido.'});
    const bytes=Buffer.from(encoded,'base64');if(bytes.length<800||bytes.length>5_200_000)return json(response,400,{error:'Audio fuera de limite.'});
    const safeMime=['audio/webm','audio/webm;codecs=opus','audio/ogg','audio/ogg;codecs=opus','audio/mp4','audio/wav'].includes(mime)?mime:'audio/webm',extension=safeMime.includes('ogg')?'ogg':safeMime.includes('mp4')?'m4a':safeMime.includes('wav')?'wav':'webm';
    const form=new FormData();form.set('model',transcriptionModel);form.set('language','es');form.set('response_format','json');form.set('file',new Blob([bytes],{type:safeMime}),`vault-note.${extension}`);
    const upstream=await fetch(`${baseUrl}/audio/transcriptions`,{method:'POST',headers:{authorization:`Bearer ${apiKey}`},body:form}),data=await upstream.json();
    if(!upstream.ok)throw new Error(data.error?.message||`Gateway ${upstream.status}`);const text=String(data.text||'').trim();if(!text)throw new Error('La transcripcion no incluyo texto.');json(response,200,{text,model:transcriptionModel});
...
async function speech(request,response){
  ...
  if(!apiKey)return json(response,503,{error:'AI_API_KEY no esta configurada en el servidor.'});
  try{
    const payload=await body(request),text=String(payload.text||'').trim();if(!text||text.length>1800)return json(response,400,{error:'Texto de voz invalido.'});
    const upstream=await fetch(`${baseUrl}/audio/speech`,{method:'POST',headers:{authorization:`Bearer ${apiKey}`,'content-type':'application/json'},body:JSON.stringify({model:speechModel,input:text,voice,response_format:'mp3'})});
    if(!upstream.ok){const data=await upstream.json().catch(()=>({}));throw new Error(data.error?.message||`Gateway ${upstream.status}`)}
    const audio=Buffer.from(await upstream.arrayBuffer());if(!audio.length)throw new Error('La voz no incluyo audio.');response.writeHead(200,{'content-type':upstream.headers.get('content-type')||'audio/mpeg','content-length':audio.length,'cache-control':'no-store'});response.end(audio);
  }catch(error){console.error('[vault-speech]',error.message);json(response,502,{error:'La voz remota no esta disponible.'})}
}

const server=createServer(async(request,response)=>{
  const url=new URL(request.url,'http://localhost');
  if(url.pathname==='/api/health'){const ai=Boolean(apiKey)&&(!isFunnelRequest(request)||allowPublicAI);return json(response,200,{ok:true,ai,model,mode:ai?'gateway':'mock',baseUrl,audio:{transcription:transcriptionModel,speech:speechModel,voice},publicAI:allowPublicAI})}
  ...
  if(url.pathname==='/api/speech'&&request.method==='POST')return speech(request,response);
...
server.listen(port,host,()=>console.log(`GNX Vault dev · http://localhost:${port} · ${apiKey?`${model} via gateway`:'mock (missing key)'}`));
