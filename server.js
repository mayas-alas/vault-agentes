...
import {clientAPI} from './whatsapp-clients.js';
...
const port=Number(process.env.PORT||4175),model=process.env.OPENAI_MODEL||'auto',effort=process.env.OPENAI_REASONING_EFFORT||'medium',apiKey=process.env.OPENAI_API_KEY||process.env.OPENAI_KEY,apiBase=(process.env.OPENAI_BASE_URL||'https://api.openai.com/v1').replace(/\/$/,'');
...
async function chat(request,response){
  ...
  try{
    ...
    const messages=[{role:'system',content:'Eres Lumen, el agente privado de GNX Vault. Responde en español, directo y cálido, máximo 90 palabras. Convierte intenciones en decisiones y siguientes pasos. Nunca afirmes que enviaste mensajes, conectaste cuentas o ejecutaste acciones si el sistema no lo confirmó.'},...history,{role:'user',content:message}];
    const upstream=await fetch(`${apiBase}/chat/completions`,{method:'POST',headers:{authorization:`Bearer ${apiKey}`,'content-type':'application/json'},body:JSON.stringify({model,messages,max_tokens:500,temperature:.35})});
