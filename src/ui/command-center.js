...
const intro='Hola, soy Lumen, el agente que acompaña esta campaña. Te escribo para presentarme y apoyar con el seguimiento y la coordinación. Cuando escriba yo, me identificaré como Lumen. ¿En qué podemos ayudarte?';
let version=0,poll;
...
export async function openCommandCenter(query=''){
  ...
  content.innerHTML=`<div class="cc-heading"><div><span class="kicker">LUMEN / CAMPAIGN WORKSPACE</span><h3 id="modal-title">Conversaciones que avanzan.</h3><p>Contexto, personas y próximos pasos, en un mismo lugar.</p></div><span class="cc-live">● WhatsApp</span></div><div class="cc-toolbar"><label class="cc-search"><span>⌕</span><input id="contact-search" autocomplete="off" placeholder="Buscar contacto, número o conversación…" aria-label="Buscar contacto"><kbd>Ctrl K</kbd></label><button id="cc-new-campaign" class="cc-secondary">+ Campaña</button></div><div id="cc-campaign-form" hidden></div><div class="cc-filters" id="cc-filters"></div><div class="cc-workspace"><aside class="cc-directory"><div class="cc-list-heading"><span id="cc-count">Cargando contactos…</span><small>↑ ↓ · Enter</small></div><div id="contact-results" role="list" aria-label="Contactos"></div></aside><section id="contact-detail" class="cc-detail" aria-label="Ficha del contacto"><div class="cc-empty"><span class="cc-orbit">L</span><h4>Cada conversación tiene contexto.</h4><p>Selecciona a alguien para ver su actividad, incorporarlo a una campaña y elegir el siguiente movimiento.</p><small>Los mensajes se envían sólo al ejecutar un flujo.</small></div></section></div><div class="cc-footer"><span id="cc-status" role="status">Conectando con tu sesión…</span><span>Esc para cerrar</span></div>`;
  ...
  let data={contacts:[],messages:[],members:{},campaigns:[]},filter='all',selected=null,selectionRun=0,visible=[],focusIndex=0,currentContext=null;
  ...
  function renderFilters(){
    ...
    filters.innerHTML=[['all','Todos'],['active','En campañas'],...data.campaigns.map(c=>[c.id,c.name])].map(([id,name])=>`<button data-filter="${escape(id)}" class="${filter===id?'active':''}" aria-pressed="${filter===id}">${escape(name)} <small>${id==='all'?data.contacts.length:Object.values(data.members).filter(m=>id==='active'||m.campaignIds?.includes(id)).length}</small></button>`).join('');
  ...
  function renderList(){
    ...
    visible=data.contacts.filter(c=>{
      ...
      return (filter==='all'||member&&(filter==='active'||member.campaignIds?.includes(filter)))&&norm(c.name+' '+c.id+' '+contactMessages(c.id).at(-1)?.text).includes(needle);
  ...
  async function refresh(){
    ...
    data={...fresh,members:fresh.members||{},campaigns:fresh.campaigns||[],contacts:[...contacts.values()]};renderFilters();renderList();
    document.dispatchEvent(new CustomEvent('campaigns-updated',{detail:{campaigns:data.campaigns,members:data.members}}));
  ...
  async function select(contact){
    ...
    try{
      ...
      currentContext=context;renderDetail(context,selection);
  ...
  function renderDetail(context,selection){
    ...
    const saveMember=async()=>{
      const campaignIds=[...detail.querySelector('#cc-campaign').selectedOptions].map(option=>option.value);
      if(!campaignIds.length){if(member){await api('membership','POST',{contactId:contact.id,remove:true});await refresh();if(active())await select(contact);}else setStatus('Elige al menos una campaña para guardar el contacto.');return;}
      await api('membership','POST',{contactId:contact.id,campaignIds,stage:detail.querySelector('#cc-stage')?.value,notes:detail.querySelector('#cc-notes').value});
    ...
    detail.querySelectorAll('[data-flow]').forEach(button=>button.onclick=()=>action(button,async()=>{
      ...
      if(!member){preview.innerHTML='<p class="cc-hint">Primero elige una campaña y agrega el contacto.</p>';return;}
      ...
      preview.innerHTML=`<div class="cc-section-title"><h5>${escape(button.querySelector('strong').textContent)}</h5><button class="cc-link" id="cc-close-flow">Cerrar</button></div>${flow==='intro'&&member.introducedAt?`<p class="cc-warning">Ya hay una presentación registrada el ${escape(date(member.introducedAt))}. Este envío la repetirá.</p>`:''}<p class="cc-hint">Para ${escape(contact.name)} · ${escape(context.campaign?.name)}</p><textarea id="send-message" rows="5" maxlength="1000" aria-label="Mensaje del flujo">${escape(text)}</textarea><div class="cc-send-row"><small>Revisa el mensaje. Tú decides cuándo enviarlo.</small><button id="confirm-send" class="cc-primary">Enviar ${flow==='intro'?'presentación':'mensaje'} ↗</button></div><p id="send-status" role="status"></p>`;
  ...
  content.querySelector('#cc-new-campaign').onclick=()=>{
    const form=content.querySelector('#cc-campaign-form');form.hidden=!form.hidden;
    if(!form.hidden){form.innerHTML='<form class="cc-create"><input name="name" maxlength="80" required placeholder="Nombre de campaña" aria-label="Nombre de campaña"><input name="goal" maxlength="1000" placeholder="¿Qué queremos lograr?" aria-label="Objetivo de campaña"><button class="cc-primary">Crear campaña</button></form>';form.querySelector('form').onsubmit=async event=>{event.preventDefault();const button=form.querySelector('button');button.disabled=true;try{await api('campaigns','POST',Object.fromEntries(new FormData(event.target)));form.hidden=true;await refresh()}catch(error){setStatus(error.message)}finally{button.disabled=false}};form.querySelector('input').focus();}
  };
