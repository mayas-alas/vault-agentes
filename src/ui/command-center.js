...
export async function openCommandCenter(query=''){
  ...
  function renderFilters(){
    ...
    filters.innerHTML=[['all','Todos'],['active','En campañas'],...data.campaigns.map(c=>[c.id,c.name])].map(([id,name])=>`<button data-filter="${escape(id)}" class="${filter===id?'active':''}" aria-pressed="${filter===id}">${escape(name)} <small>${id==='all'?data.contacts.length:Object.values(data.members).filter(m=>id==='active'||m.campaignId===id).length}</small></button>`).join('');
  ...
  function renderList(){
    ...
    visible=data.contacts.filter(c=>{
      ...
      return (filter==='all'||member&&(filter==='active'||member.campaignId===filter))&&norm(c.name+' '+c.id+' '+contactMessages(c.id).at(-1)?.text).includes(needle);
  ...
  function renderDetail(context,selection){
    ...
    const saveMember=async()=>{
      const campaignId=detail.querySelector('#cc-campaign').value;
      if(!campaignId){if(member){await api('membership','POST',{contactId:contact.id,remove:true});await refresh();if(active())await select(contact);}else setStatus('Elige una campaña para guardar el contacto.');return;}
      await api('membership','POST',{contactId:contact.id,campaignId,stage:detail.querySelector('#cc-stage')?.value,notes:detail.querySelector('#cc-notes').value});
