...
export async function openCommandCenter(query=''){
  ...
  let data={contacts:[],messages:[],members:{},workspaces:[]},filter='active',selected=null,selectionRun=0,visible=[],focusIndex=0,currentContext=null;
  ...
  function renderList(){
    ...
    visible=data.contacts.filter(c=>{
      ...
      const included=filter==='active'?(Boolean(member)||Boolean(needle&&!member)):filter==='available'?!member:Boolean(member?.workspaceIds?.includes(filter));
      return included&&norm(c.name+' '+c.id+' '+contactMessages(c.id).at(-1)?.text).includes(needle);
  ...
  function renderDetail(context,selection){
    ...
    const mergeCard=document.createElement('div');mergeCard.className='cc-merge';mergeCard.innerHTML='<button class="cc-link" id="cc-merge-open">Unir este contacto con otro</button><div id="cc-merge-form" hidden><select id="cc-merge-target" aria-label="Contacto duplicado"><option value="">Selecciona el contacto que representa lo mismo…</option>'+data.contacts.filter(item=>item.id!==contact.id).map(item=>`<option value="${escape(item.id)}">${escape(item.name||item.id)} · ${escape(item.id.split('@')[0])}</option>`).join('')+'</select><button class="cc-secondary" id="cc-merge-confirm">Unir historial</button><small>Conservaremos ambos historiales en este contacto.</small></div>';
    detail.querySelector('.cc-membership')?.append(mergeCard);
    if(!member&&data.workspaces[0]){
      const activate=document.createElement('button');activate.className='cc-primary cc-activate-contact';activate.textContent='Activar y enviar saludo de Lumen ↗';detail.querySelector('.cc-membership')?.append(activate);
      activate.onclick=async()=>{activate.disabled=true;activate.textContent='Activando…';try{await api('membership','POST',{contactId:contact.id,workspaceIds:[data.workspaces[0].id],stage:'active',notes:''});const result=await api('send','POST',{contactId:contact.id,message:intro,flow:'intro',requestId:crypto.randomUUID()});setStatus(`Saludo de Lumen enviado a ${contact.name} · ${result.messageId}`);await refresh();if(active())await select(contact)}catch(error){activate.disabled=false;activate.textContent='Reintentar activación';setStatus(error.message)}};
    }
    mergeCard.querySelector('#cc-merge-open').onclick=()=>{mergeCard.querySelector('#cc-merge-form').hidden=false;mergeCard.querySelector('#cc-merge-target').focus()};
    mergeCard.querySelector('#cc-merge-confirm').onclick=async()=>{const duplicateId=mergeCard.querySelector('#cc-merge-target').value;if(!duplicateId)return;try{await api('merge','POST',{primaryId:contact.id,duplicateId});setStatus('Historiales unidos');await refresh();if(active())await select(contact)}catch(error){setStatus(error.message)}};
