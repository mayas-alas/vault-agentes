# Plan de contexto para espacios y conversaciones
...
El primer ajuste ya implementado en el MVP agrupa aliases conocidos de WhatsApp en el worker y migra membresías antiguas de `workspaceId` a `workspaceIds[]`. Lo que sigue abajo conserva el diseño de validación para completar la sincronización en vivo y cubrir aliases que WhatsApp entregue después.

1. La conversación debe mostrar todos los mensajes que WhatsApp haya sincronizado, incluida la respuesta del usuario.
2. Un contacto puede pertenecer a varias espacios sin duplicar el contacto ni perder el historial.
...
La pertenencia de espacio actual tiene la forma `members[contactId] = { workspaceId, stage, notes }`. Eso impide representar dos espacios para el mismo contacto y hace que el filtro de espacio sea ambiguo.
...
La pantalla debe mostrar una tarjeta por contacto y dentro de ella chips de todas sus espacios. El filtro de espacio devuelve el contacto si contiene esa espacio; nunca debe duplicar la tarjeta por cada espacio.

Las acciones de flujo reciben siempre `canonicalContactId` y `workspaceId`. El mensaje enviado se registra con ambos valores, `messageId`, `requestId`, estado de WhatsApp y fecha. Reintentar el mismo `requestId` debe devolver el resultado guardado y no enviar otro mensaje.
...
El resumen se genera por par `canonicalContactId + workspaceId`, con una revisión hash del historial, notas y objetivo de espacio. Debe contener:

- contexto y objetivo de la espacio;
...
Una presentación de Lumen sólo prueba que el mensaje salió. No prueba lectura, respuesta, interés ni incorporación efectiva a una espacio.
...
Cada flujo muestra destinatario, espacio, contexto usado, textarea editable, botón de envío, request ID y estado confirmado por WhatsApp.

## Criterios de aceptación

- El historial de Albracom muestra el mensaje de Lumen y cualquier respuesta recibida aunque llegue con otro JID.
- Un contacto puede pertenecer a dos espacios y aparecer una sola vez en `Todos`.
- Cambiar de espacio conserva notas e historial, pero genera resúmenes independientes.
...
- Las pruebas cubren JID alterno, multi-espacio, actualización en vivo, idempotencia de envío y diseño móvil.

## Orden de implementación

1. Canonicalizar JIDs y migrar mensajes existentes.
2. Migrar `workspaceId` a `workspaces[]` con compatibilidad de lectura.
